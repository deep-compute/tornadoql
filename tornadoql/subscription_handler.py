...
class GQLSubscriptionHandler(websocket.WebSocketHandler):
    ...
    def on_start(self, op_id, params):
        try:
            ...
            subscription = execution_result.subscribe(
                SubscriptionObserver(
                    op_id, self.send_execution_result, self.send_error, self.on_close
                )
            )
    ...
    def unsubscribe(self, op_id):
        self.log.debug(
            "unsubscribe_called", op_id=op_id, nsubscriptions=len(self.subscriptions)
        )
